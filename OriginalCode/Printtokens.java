...
public class Printtokens{
	...
	static BufferedReader open_character_stream(String fname) { //
		...
		// cannot check for fname = null but checking if no file was passed
		if (fname.equals("")) {
	...
	/********************************************************/
	BufferedReader open_token_stream(String fname) //need object instance to test
	{
		BufferedReader br;
	if(fname.equals("")) //removed the check for fname==null
	    br=open_character_stream(""); //corrected null value passed to an empty string
	 else
	...
	{
//	  int i=0,j; //no use to our program
	  int id=0;
	  int res = 0;
	  char ch = '\0';
	  ...
	  StringBuilder sb = new StringBuilder();
	   ...
	   sb.append(ch);
	   if(is_spec_symbol(ch))return sb.toString();
	...
	static boolean is_token_end(int str_com_id, int res)
	{
	 ...
	 if(is_spec_symbol(ch)) return true; /* is special_symbol? */
	...
	{
	  int i=1; //during testing, i had to change this to 3 to test triple digits

	  if ( Character.isDigit(str.charAt(0)))
	    {
		while ( i < str.length() && str.charAt(i) != '\0' )
	      {
//			  found error while testing this method with "3a" parameter, fixed
...
//			bug found should be false
	...
	public static void main(String[] args)  {
		...
		if (args.length == 0) {	/* if not given filename,take as '""' */
			fname = ""; //changed from new String() a check for no file ""
